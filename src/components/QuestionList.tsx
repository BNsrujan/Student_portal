      ...
      <div className="space-y-3">
        {questions.map((question, index) => {
          ...
          return (
            <div
              key={question.id}
              className={`p-4 rounded-xl transition-all duration-300 transform hover:scale-102 ${isAnswered
            ...
            >
              <div className="flex items-center space-x-3">
                <span className={`w-8 h-8 flex items-center justify-center rounded-full ${isAnswered
                    ? 'bg-green-400 dark:bg-green-500 text-white'
                    : 'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300'
                  }`}>
                  {index + 1}
                </span>
                <span className={`text-sm font-medium ${isAnswered ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-gray-300'
                  }`}>
                  Question {index + 1}
                  {isAnswered && (
